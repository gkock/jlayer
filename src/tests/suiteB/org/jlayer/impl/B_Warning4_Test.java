...
/**
 * checks the {@code org.jlayer.impl.Unit} class objects generated from {@link faulty.units.Warning4}
 * 
 * @author Gerd Kock
 */
@Test(groups = {"suiteB"})
public class B_Warning4_Test {
	
	// All classes that evoke warnings 
	String unit = userDir + "/src/units/faulty/units/Warning4.java";
    ...
    public void testWarning() {
		...
		assertEquals(processedUnit.typeDescr.typeName.toString(), "Warning4");
		assertEquals(processedUnit.fields.size(), 2);
		assertEquals(processedUnit.methods.size(), 0);
    }
    
    @AfterClass
    public void afterClass() {
    	List<Diagnostic<? extends JavaFileObject>> dList = diagnosticCollector.getDiagnostics();
    	assertEquals(dList.size(), 10);
    	// TEST OUTPUT
