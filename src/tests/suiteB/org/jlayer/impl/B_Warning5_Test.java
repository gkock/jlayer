 ...
 */
@Test(groups = {"suiteB"})
public class B_Warning5_Test {
	
	// All classes that evoke warnings 
	String unit = userDir + "/src/units/faulty/units/Warning5.java";
    ...
    public void testWarning() {
		...
		assertEquals(processedUnit.typeDescr.typeName.toString(), "Warning5");
		assertEquals(processedUnit.fields.size(), 1);
		assertEquals(processedUnit.methods.size(), 2);
    }
    
    @AfterClass
    public void afterClass() {
    	List<Diagnostic<? extends JavaFileObject>> dList = diagnosticCollector.getDiagnostics();
    	assertEquals(dList.size(), 4);
    	// TEST OUTPUT
