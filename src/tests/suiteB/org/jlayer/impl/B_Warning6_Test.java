...
/**
 * checks the {@code org.jlayer.impl.Unit} class objects generated from {@code faulty.units.Warning5},
 * 
 * @author Gerd Kock
 */
@Test(groups = {"suiteB"})
public class B_Warning6_Test {
	
	// All classes that evoke warnings 
	String unit = userDir + "/src/units/faulty/units/Warning6.java";
    ...
    public void testWarning() {
		...
		assertEquals(processedUnit.typeDescr.typeName.toString(), "Warning6");
		assertEquals(processedUnit.fields.size(), 1);
		assertEquals(processedUnit.methods.size(), 1);
    }
    
    @AfterClass
    public void afterClass() {
    	List<Diagnostic<? extends JavaFileObject>> dList = diagnosticCollector.getDiagnostics();
    	assertEquals(dList.size(), 13);
    	// TEST OUTPUT
