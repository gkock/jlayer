...
/**
 * checks the {@code org.jlayer.impl.Unit} class object generated from {@link basic.units.AbstractUnit}.
 * @author Gerd Kock
 */
@Test(groups = {"suiteB"})
public class B_AbstractUnit_Test {
	
	String unit = userDir + "/src/units/basic/units/AbstractUnit.java";
	Unit processedUnit;
    ...
    public void testBasics() {
    	...
    	assertEquals(processedUnit.typeDescr.typeName.toString(), "AbstractUnit");
    	assertEquals(processedUnit.typeDescr.modifiers.size(), 2);
    	assertTrue(processedUnit.typeDescr.modifiers.contains(Modifier.ABSTRACT));
    	assertTrue(processedUnit.typeDescr.modifiers.contains(Modifier.PUBLIC));
    }
    ...
    public void testFieldsOfUnit() {
    	...
    	for (FieldDescriptor fDescr : processedUnit.fields) {
    		...
    		assertTrue(fDescr.modifiers.contains(Modifier.PROTECTED));
		}
    }
