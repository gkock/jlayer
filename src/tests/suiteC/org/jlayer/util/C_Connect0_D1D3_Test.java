...
/**
 * Testing {@code VectorLayer*.D1.connect(Layer.D3<?,T> layer, Relation rel)}.
 *
 * @author Gerd Kock
 */
@Test(groups = {"suiteC"})
public class C_Connect0_D1D3_Test {
...
//	final int sz2 = 4;
	...
	public void testVectorLayerNN1full() {
		...
		unit1layerD1.intVector.connect(unit2layerD3.intSignal, full);
		// check the link vectors
		for (int x1 = 0; x1 < sz1; x1++){
			Unit1.SignalInt[] ivector = unit1layerD1.intVector.get(x1);
		...
		for (int x1 = 0; x1 < sz1; x1++){
			for (int y = 0; y < sz3*sz3*sz3; y++) {
				assertEquals(unit1layerD1.intVector.get(x1)[y].getVal(), val27_1[y]);
			}
		}
		
		// associate using relation rand
		unit1layerD1.doubleVector.connect(unit2layerD3_doubleSignal, rand);
		// check the link vectors
		for (int x1 = 0; x1 < sz1; x1++){
			Unit1.SignalDouble[] dvector = unit1layerD1.doubleVector.get(x1);
		...
		unit1layerD1_unit1Link.connect(unit2layerD3.unit1Item, xHalf);
		// check the link vectors
		for (int x1 = 0; x1 < sz1; x1++){
			Unit1[] unit1Link = unit1layerD1_unit1Link.get(x1);
		...
		unit1layerD1_unit2Link.connect(unit2layerD3_unit2Item, yHalf);
		// check the connected signals
		for (int x1 = 0; x1 < sz1; x1++){
			Unit2[] unit2Link = unit1layerD1_unit2Link.get(x1);
		...
		for (int x1 = 0; x1 < sz1; x1++){
			...
			for (int y = 0; y < unit2Link.length; y++) {
				assertEquals(unit1layerD1_unit2Link.get(x1)[y].noItem, val27[y]);
			}
		}		
