...
/**
 * Testing {@code VectorLayer*.D3.connect(Layer.D3<?,T> layer, Relation rel)}.
 *
 * @author Gerd Kock
 */
@Test(groups = {"suiteC"})
public class C_Connect1_D3D3_Test {
...
//	final int sz2 = 4;
	...
	public void testVectorLayerNN1full() {
		...
		unit1layerD3.intVector.connect(unit2layerD3.intSignal, full);
		...
		for (int x1 = 0; x1 < sz3; x1++){
			for (int x2 = 0; x2 < sz3; x2++){
				for (int x3 = 0; x3 < sz3; x3++){
					for (int y = 0; y < sz3*sz3*sz3; y++) {
						assertEquals(unit1layerD3.intVector.get(x1, x2, x3)[y].getVal(), val27_1[y]);
					}
				}
			}
		}
		...
		unit1layerD3.doubleVector.connect(unit2layerD3_doubleSignal, rand);
		// check the link vectors
		for (int x1 = 0; x1 < sz3; x1++){
			for (int x2 = 0; x2 < sz3; x2++){
				for (int x3 = 0; x3 < sz3; x3++){
		...
		unit1layerD3_unit1Link.connect(unit2layerD3.unit1Item, xHalf);
		// check the link vectors
		for (int x1 = 0; x1 < sz3; x1++){
			for (int x2 = 0; x2 < sz3; x2++){
				for (int x3 = 0; x3 < sz3; x3++){
		...
		unit1layerD3_unit2Link.connect(unit2layerD3_unit2Item, yHalf);
		...
		for (int x1 = 0; x1 < sz3; x1++){
			for (int x2 = 0; x2 < sz3; x2++){
				for (int x3 = 0; x3 < sz3; x3++){
					...
					for (int y = 0; y < unit2Link.length; y++) {
						assertEquals(unit1layerD3_unit2Link.get(x1, x2, x3)[y].noItem, val27[y]);
					}
				}
			}
		}		
