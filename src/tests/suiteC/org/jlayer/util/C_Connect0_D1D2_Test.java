...
/**
 * Testing {@code VectorLayer*.D1.connect(Layer.D2<?,T> layer, Relation rel)}.
 *
 * @author Gerd Kock
 */
@Test(groups = {"suiteC"})
public class C_Connect0_D1D2_Test {
...
//	final int sz3 = 3;
	...
	public void testVectorLayerNN1full() {
		...
		unit1layerD1.intVector.connect(unit2layerD2.intSignal, full);
		...
		for (int x1 = 0; x1 < sz1; x1++){
			for (int y = 0; y < sz2*sz2; y++) {
				assertEquals(unit1layerD1.intVector.get(x1)[y].getVal(), val16_1[y]);
			}
		}
		
		// associate using relation rand
		unit1layerD1.doubleVector.connect(unit2layerD2_doubleSignal, rand);
		// check the link vectors
		for (int x1 = 0; x1 < sz1; x1++){
		...
		unit1layerD1_unit1Link.connect(unit2layerD2.unit1Item, xHalf);
		// check the link vectors
		for (int x1 = 0; x1 < sz1; x1++){
		...
		unit1layerD1_unit2Link.connect(unit2layerD2_unit2Item, yHalf);
		...
		for (int x1 = 0; x1 < sz1; x1++){
			...
			for (int y = 0; y < unit2Link.length; y++) {
				assertEquals(unit1layerD1_unit2Link.get(x1)[y].noItem, val16[y]);
			}
		}		
