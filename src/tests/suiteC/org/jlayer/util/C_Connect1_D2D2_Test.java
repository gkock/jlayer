...
/**
 * Testing {@code VectorLayer*.D2.connect(Layer.D2<?,T> layer, Relation rel)}.
 *
 * @author Gerd Kock
 */
@Test(groups = {"suiteC"})
public class C_Connect1_D2D2_Test {
...
//	final int sz3 = 3;
	...
	public void testVectorLayerNN1full() {
		...
		unit1layerD2.intVector.connect(unit2layerD2.intSignal, full);
		...
		for (int x1 = 0; x1 < sz2; x1++){
			for (int x2 = 0; x2 < sz2; x2++){
				for (int y = 0; y < sz2*sz2; y++) {
					assertEquals(unit1layerD2.intVector.get(x1, x2)[y].getVal(), val16_1[y]);
				}
			}
		}
		
		// connect using relation rand
		unit1layerD2.doubleVector.connect(unit2layerD2_doubleSignal, rand);
		// check the link vectors
		for (int x1 = 0; x1 < sz2; x1++){
			for (int x2 = 0; x2 < sz2; x2++){
		...
		unit1layerD2_unit1Link.connect(unit2layerD2.unit1Item, xHalf);
		// check the link vectors
		for (int x1 = 0; x1 < sz2; x1++){
			for (int x2 = 0; x2 < sz2; x2++){
		...
		for (int x1 = 0; x1 < sz2; x1++){
			for (int x2 = 0; x2 < sz2; x2++){
				...
				for (int y = 0; y < unit1Link.length; y++) {
					// FEHLERSUCHE
					int actual = unit1Link[y].noItem;
					int soll = val16_1[y];
					if ( (actual==3) && (soll!=3) ) {
						System.out.println(" HERE WE ARE unit1Link");
					} else {
						assertEquals(actual, soll);
					}
		...
		unit1layerD2_unit2Link.connect(unit2layerD2_unit2Item, yHalf);
		...
		for (int x1 = 0; x1 < sz2; x1++){
			for (int x2 = 0; x2 < sz2; x2++){
				...
				for (int y = 0; y < unit2Link.length; y++) {
					// FEHLERSUCHE
					int actual = unit1layerD2_unit2Link.get(x1, x2)[y].noItem;
					int soll = val16[y];
					if ( (actual==7) && (soll!=7) ) {
						System.out.println(" HERE WE ARE unit2Link");
					} else {
						assertEquals(actual, soll);
					}
					assertEquals(unit1layerD2_unit2Link.get(x1, x2)[y].noItem, val16[y]);
				}
			}
		}		
