 ...
 * here layers of dims == 1 and 3
 * 
 * @author Gerd Kock
 */
@Test(groups = {"suiteC"})
public class C_Signal13_Test {
	...
	public void testConnect() {
		...
		for (int i=0; i < sz3; i++) {
			...
			assertEquals(sum(inLayer.connectedVector.get(i)), 12);
			assertEquals(sum(connectedVectorLayer.get(i)), 12);
		}
		...
		for (int i=0; i < sz3; i++) {
			...
			assertEquals(sum(inLayer.connectedVector.get(i)), 12 * 2);
			assertEquals(sum(connectedVectorLayer.get(i)), 12 * 2);
		}
		...
		for (int i=0; i < sz3; i++) {
			...
			assertEquals(sum(inLayer.connectedVector.get(i)), 16 * 8 + 12 * 2);
			assertEquals(sum(connectedVectorLayer.get(i)), 16 * 8 + 12 * 2);
		}
	}
	...
	public void testAssociate() {
		...
		for (int i=0; i < sz3; i++) {
			...
			assertEquals(sum(inLayer.associatedVector.get(i)), 12);
			assertEquals(sum(associatedVectorLayer.get(i)), 12);
		}
		...
		for (int i=0; i < sz3; i++) {
			...
			assertEquals(sum(inLayer.associatedVector.get(i)), 12);
			assertEquals(sum(associatedVectorLayer.get(i)), 12 * 2);
		}
		...
		for (int i=0; i < sz3; i++) {
			...
			assertEquals(sum(inLayer.associatedVector.get(i)), 8 + 12);
			assertEquals(sum(associatedVectorLayer.get(i)), 8 + 12 * 2);
		}
	}
